...
def construct_message(message_id, payload):
    message_id = message_id.to_bytes(1, byteorder="big")  # Specify byteorder
...
def request_block(s, piece_index, block_index, length):
    ...
    received_index = int.from_bytes(piece_message[5:9], byteorder="big")  # Specify byteorder
    received_begin = int.from_bytes(piece_message[9:13], byteorder="big")  # Specify byteorder
...
def download_piece(outputfile, filename, piececount):
    ...
    # Write piece to disk using a temporary directory
    temp_dir = tempfile.gettempdir()
    temp_file = os.path.join(temp_dir, f"test-{piececount}")
    with open(temp_file, "wb") as piece_file:
        piece_file.write(piece)
    # Clean up
    s.close()
    # Return piece completed and location
    return piececount, temp_file
